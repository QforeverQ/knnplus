...
//无穷范数
void dminf(double *learn, double *valid, int *n, int *m, int *p, double *dm, int *cl, int *k, double *weights){
  int i, j, l, t, nn, ii, kk;
  double tmp, tmp2, *dvec, maxD;
  int *cvec;
  ...
  for(j=0;j<(*m);j++){
    ...
    while(i<*n){
      ...
      l=0;
      while(l<*p && tmp < (maxD+EPS)){
        tmp2 = fabs(learn[i+l*n[0]]-valid[j+l*m[0]])*weights[l];
        tmp=fmax2(tmp2,tmp);
        l++;
      }
      if(tmp<maxD){
