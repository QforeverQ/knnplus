.onLoad  <- function(libname, pkgname) {
    library.dynam ("knnplus", pkgname, libname)
}
...
# ordering allows branch and bound in distance computation
    ...
    Euclid <- 1
    if(distance==2) Euclid <- 2
    if(distance==0) Euclid <- 0
    if(Euclid == 2) dmtmp <- .C("dmEuclid", as.double(learn), as.double(valid), 
        as.integer(m), as.integer(p), as.integer(q), 
        dm=double((k+1L) * p), cl=integer((k+1L) * p), k=as.integer(k+1), 
        as.double(we), PACKAGE='kknn')

    else if(Euclid == 0) dmtmp <- .C("dminf", as.double(learn), as.double(valid),
                                     as.integer(m),as.integer(p), as.integer(q), 
                                     dm=double((k+1L) * p), cl=integer((k+1L) * p), 
                                     k=as.integer(k+1), 
                                     as.double(we), PACKAGE='kknn')
...
# ordering
    ...
    kmax2 <- kmax + 2L
    if(kmax2 > m) kmax2 <- m
    if(Euclid==2) dmtmp <- .C("dmEuclid", as.double(mm.data), as.double(mm.data), 
        as.integer(m), as.integer(p), as.integer(q), 
        dm = double((kmax2) * p), cl = integer(kmax2 * p), 
        k = as.integer(kmax2), as.double(we), PACKAGE = "kknn")
    else if(Euclid==0) dmtmp <- .C("dminf", as.double(mm.data), as.double(mm.data), 
                                   as.integer(m), as.integer(p), as.integer(q), 
                                   dm = double((kmax2) * p), cl = integer(kmax2 * p), 
                                   k = as.integer(kmax2), as.double(we), PACKAGE = "kknn")
...
#' use KNN model to fit train data and predict test data,and show the plot
#' 
#' @param train_data DataFrame of explanatory variable in train data
#' @param train_y DataFrame of response variable in train data
#' @param test_data
#' @param test_y
#' @param y_type str,"continuous"(default) or "descrete"
#' @param cv_num int,
#' @param k_max int, max number of features selected
#' @param distance_num type of norm
#' @param kernel kernel function of weight
#' @return the result of prediction, analyis and plot of the result
knn_model<-function(train_data,train_y,test_data,test_y,
                    y_type='continuous',cv_num=10,
                    k_max=10,
                    ditance_num=2,   
                    kernel='optimal'){
  library(lattice)
  ...
  train_data<-transform(train_data, y=train_y)
  
  kknn_control <- trainControl(method = 'cv',  
                               number = cv_num)

  kknnGrid <- expand.grid(kmax = seq(from = 1, to = k_max),
                          distance = ditance_num,
                          kernel = kernel)
  
  kknn_model<-train(y ~ .,data = train_data,
                    method = 'kknn',
                    tuneGrid = kknnGrid,
                    preProcess = c('center','scale'),
                    trControl = kknn_control)
 
  plot_data <- kknn_model$results
  best_k <- kknn_model$bestTune$kmax
  if(y_type=='continuous'){
    best_RMSE <- kknn_model$results$RMSE[best_k]
    k_plot<-ggplot(data = plot_data,
                   mapping = aes(x = kmax, y = RMSE)) +
      geom_line(color = 'steelblue', lwd = 1) +
      geom_point(color = 'red', size = 3) +
      scale_x_continuous(breaks = seq(from = 1, to = 10)) +
      geom_text(mapping = aes(x = best_k, y = best_RMSE+0.03,
                              label = paste0('RMSE:',round(best_RMSE,2))))+
      labs(x='k')
  }
  else{
    best_accuracy <- kknn_model$results$Accuracy[best_k]
    k_plot<-ggplot(data = plot_data,
                   mapping = aes(x = kmax, y = Accuracy)) +
      geom_line(color = 'steelblue', lwd = 1) +
      geom_point(color = 'red', size = 3) +
      scale_x_continuous(breaks = seq(from = 1, to = 10)) +
      geom_text(mapping = aes(x = best_k+1, y = best_accuracy,
                              label = paste0('Accuracy:',round(best_accuracy,2))))+
      labs(x='k')
    
  }
  
  pred <- predict(object = kknn_model, newdata = test_data)
  if (y_type=='continuous'){
    RMSE <- sqrt(mean((pred - test_y) ** 2))
    real_pred<-data.frame(real=test_y,prediction=pred,residual=(test_y-pred))
    pred_plot<-ggplot(data=real_pred,
                      mapping = aes(x = test_y, y = pred)) +
      geom_point(color = 'red', size = 1)+
      geom_line(aes(real,real),color = 'steelblue', lwd = 1)+
      labs(x='real')
    residual_plot<-ggplot(data=real_pred,
                          mapping=aes(x=seq(1:length(test_y)),y=residual))+
      geom_point(color = 'red', size = 1)+
      labs(x='Index')
    result<-list(kknn_model=kknn_model,k_plot=k_plot,
                 pred_plot=pred_plot,RMSE=RMSE,
                 real_pred=real_pred,
                 residual_plot=residual_plot)
  }
  else{
    label = unique(as.vector(unlist(test_y)))
    freq = table(pred, real = test_y)
    cm <- confusionMatrix(freq, mode = 'prec_recall')
    cm_data <- as.data.frame(freq)
    cm_data$pred <- factor(cm_data$pred,
                           levels = label,
                           ordered = TRUE)
    cm_data$real <- factor(cm_data$real,
                           levels = rev(label),
                           ordered = TRUE)
    picture<-ggplot(data = cm_data,
                    mapping = aes(x = pred, y = real, fill = Freq)) +
      geom_tile() +
      scale_fill_continuous(low = 'lightblue',
                            high = 'darkblue',
                            guide = FALSE) +
      geom_text(aes(x = pred, y = real, label = Freq),
                color = 'white')
    accracy <- cm$overall[1]
    recall<-t(cm$byClass)[3,]
    result<-list(kknn_model=kknn_model,k_plot=k_plot,
                 cm=cm,cm_pic=picture,accracy=accracy,
                 recall=recall)
  }
  return(result)
}
